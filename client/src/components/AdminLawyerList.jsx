...
export default function AdminLawyerList({
  lawyers = [],
  onLawyerClick,
}) {
  ...
  const approvedLawyers = lawyers.filter(lawyer => lawyer.verification === "Approved");

  // Get unique specializations
  const allSpecializations = [...new Set(approvedLawyers.flatMap(l => l.specializations || []))].sort();
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
        <div className="flex flex-wrap items-center gap-2">
          <div className="p-3 bg-emerald-100 rounded-lg text-emerald-600"><Shield size={20} /></div>
          <h2 className="text-xl font-semibold text-gray-800">Total Lawyers</h2>
          <span className="px-3 py-1 text-xs font-semibold bg-green-100 text-green-700 rounded-full">{approvedLawyers.length} Approved</span>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <select 
          ...
          className="pl-4 pr-8 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 appearance-none bg-white cursor-pointer"
        >
      ...
      {approvedLawyers.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-10 text-center">
          ...
          <p className="text-gray-500">No approved lawyers found.</p>
        </div>
      ) : (
