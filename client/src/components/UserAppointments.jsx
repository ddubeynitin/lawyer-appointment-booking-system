...
export default function UserAppointments() {
  ...
  const getLawyerInfo = (lawyerId) => {
    ...
    // Check if lawyerId is already populated with name
  ...
  // Show all appointments (no filter by current user)
  ...
  }, [userAppointments, users, lawyers, searchQuery, statusFilter]);

  const totalAppointments = userAppointments.length;
  const pendingCount = userAppointments.filter(a => a?.status === "Pending").length;
  const approvedCount = userAppointments.filter(a => a?.status === "Approved").length;
  const rejectedCount = userAppointments.filter(a => a?.status === "Rejected").length;
